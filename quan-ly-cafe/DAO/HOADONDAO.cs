...
namespace quan_ly_cafe.DAO
{
    public class HOADONDAO
    {
        ...
        public DataTable doanhthutheonhanvien(string user, string ngayban)
        {
            string sql = "SELECT MON.TENMON, SUM(HOADON.SOLUONG) AS SOLUONG, SUM(HOADON.THANHTIEN) AS THANHTIEN FROM MON, HOADON WHERE MON.MAMON = HOADON.MAMON AND USERNAME = '" + user + "' AND NGAYBAN = '"+ngayban+"'  GROUP BY HOADON.MAMON, MON.TENMON ";
            DataTable result = Dataprovider.Instance.Query(sql);
            return result;
        }
        //Cập nhật trạng thái đã thanh toán
        public bool capnhatthanhtoan(int maban,string userthungan)
        {
            string sql = "update HOADON set THANHTOAN = 1, USERNAMETN = '"+userthungan+"'  where MABAN = '" + maban + "' ";
            int result = Dataprovider.Instance.ExNonQuery(sql);
            return result > 0;
        }
